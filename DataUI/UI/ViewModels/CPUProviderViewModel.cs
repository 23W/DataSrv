...
{
    public class CPUProviderViewModel : ObservableModel
    {
        #region Properties

        public IWindowViewModelHost? Host { get; private set; }

        public CPUTempData? Provider { get; private set; } = default;

        public IEnumerable<SourceViewModel> Sources => GetSourceViewModels();

        public SourceType Source
        {
            get => Provider?.Source ?? SourceType.PDH;
            set
            {
                if (Provider == default)
                {
        ...
        internal static CPUProviderViewModel Create(CPUTempData provider, IWindowViewModelHost? host)
        {
            var res = new CPUProviderViewModel()
            {
                Provider = provider,
                Host = host
            };
        ...
        void BindTextTitels()
        {
            ...
            if (axisTemp != default)
            {
                axisTemp.Title = (string)Host.ApplicationResources["CPUTemperatureLabel"];
